...
class GenericMask:
    ...
    def __init__(self, mask_or_polygons, height, width):
        ...
        if isinstance(m, np.ndarray):  # assumed to be a binary mask
            ...
            assert m.shape == (height, width), (m.shape, (height, width))
...
class Visualizer:
    ...
    def draw_instance_gt(self, predictions):
        ...
        labels = _create_text_labels(classes, scores, self.metadata.get("thing_classes", None))
        keypoints = predictions.gt_keypoints if predictions.has("pred_keypoints") else None
        ...
        if self._instance_mode == ColorMode.IMAGE_BW:
            assert predictions.has("gt_masks"), "ColorMode.IMAGE_BW requires segmentations"
            self.output.img = self._create_grayscale_image(
